...
function buildReportHtml(){
  ...
  <div class="report-kpis">
    ...
    <div class="report-kpi"><small>Total Issues</small><strong>${Object.values(counts).reduce((a,b)=>a+b,0)}</strong></div>
  ...
  <div class="report-section"><h3>Page Summary</h3><table class="report-table"><thead><tr><th>URL</th><th>Status</th><th>Words</th><th>Load</th><th>Issues</th></tr></thead><tbody>${ps.map(p=>`<tr><td>${escapeHtml(p.url)}</td><td>${p.status_code??0}</td><td>${p.word_count??0}</td><td>${p.response_time_ms??0} ms</td><td>${escapeHtml((p.issues||[]).join(', ')||'None')}</td></tr>`).join('')}</tbody></table></div>
...
function downloadReport(){if(!pages().length){generateReport();if(!pages().length)return;}const title=($('#reportTitle')?.value||'GEORUSH SEO Audit Report').replace(/[^\w-]+/g,'-');const doc=`<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>body{font:14px Arial;margin:40px;color:#172535}table{width:100%;border-collapse:collapse}th,td{border-bottom:1px solid #ddd;padding:8px;text-align:left}</style></head><body>${buildReportHtml()}</body></html>`;downloadBlob(`${title}.html`,'text/html;charset=utf-8',doc);}
function downloadReportCsv(){
  const ps=pages();if(!ps.length){return;}
  const lines=[['URL','Status','Title','Words','Load ms','Issues'].join(',')];
  ps.forEach(p=>lines.push([p.url,p.status_code,p.title||'',p.word_count||0,p.response_time_ms||0,(p.issues||[]).join('|')].map(v=>`"${String(v).replace(/"/g,'""')}"`).join(',')));
  downloadBlob('GEORUSH-audit-pages.csv','text/csv;charset=utf-8',lines.join('\n'));
}
function printReport(){if(!pages().length){generateReport();if(!pages().length)return;}const w=window.open('','_blank');if(!w){alert('Allow pop-ups for GEORUSH to print the report.');return;}w.document.write(`<!doctype html><html><head><title>GEORUSH SEO Report</title><style>body{font:14px Arial;margin:40px;color:#172535}.report-kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}.report-kpi{border:1px solid #ddd;padding:12px}.report-kpi small{display:block;color:#667}.report-kpi strong{font-size:24px}table{width:100%;border-collapse:collapse}th,td{border-bottom:1px solid #ddd;padding:7px;text-align:left}</style></head><body>${buildReportHtml()}</body></html>`);w.document.close();w.focus();setTimeout(()=>w.print(),300);}
